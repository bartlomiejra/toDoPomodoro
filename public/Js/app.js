...
<button  tabindex="0" class="center_delete-btn" aria-label="Delete" data-index=${
                todo.id
              } id="item${
                todo.id
      ...
      db.collection('users')
        ...
        .onSnapshot((querySnapshot) => {
          ...
          for (let i = 0; i < historylist.length; i += 1) {
            if (
              historylist[i].done == true &&
              historylist[i].data == dateToday
            ) {
              dbcountCompleted += 1;
            }
          }
          ...
          db.collection('users')
            ...
            .onSnapshot((querySnapshot) => {
              ...
              for (let i = 0; i < todoListst.length; i += 1) {
                if (
                  todoListst[i].done == false &&
                  todoListst[i].data == dateToday
                ) {
                  dbtoBeCompleted += 1;
                }
              }
          ...
          if (clickedTodo.repeatday != 0) {
            ...
            db.collection('users')
              ...
              .then((querySnapshot) => {
                ...
                const newData = moment(clickedTodo.data)
                  .add(rep, repTime)
                  .format('YYYY-MM-DD');
...
function sortingProject(clicked_id) {
  ...
  db.collection('users').doc(logUserId).collection('Current').doc('0').set({
    id: 0,
    name: clicked,
  });
...
function sortingProjectDays(e) {
  ...
  db.collection('users')
    ...
    .then((querySnapshot) => {
      ...
      switch (clicked) {
        case 'Today':
          taskToday = allProjectList.filter(
            (items) => items.data === dateToday,
          );
          db.collection('users')
            .doc(logUserId)
            .collection('Current')
          ...
          break;
        case 'Tomorrow':
          taskTomorrow = allProjectList.filter(
